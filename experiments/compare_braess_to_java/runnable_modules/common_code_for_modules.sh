...
run_callback_for_parameter_arrays_parallel() {
  ...
  local max_parallel_jobs="$9"
  local actual_callback="${10}"
  ...
  local callback_q experiment_set_name_q base_output_dir_q experiment_output_dir_pattern_q delete_output_dir_if_existing_q common_q arg extra_args=""
 ...
 # TODO at some point, add --memfree *smth* to parallel to avoid running out of memory when running many jobs in parallel.
  ...
  parallel --will-cite --eta --bar -j "${max_parallel_jobs}" \
    "bash -lc 'source ${common_q}; source ${module_q}; ${callback_q} {1} {2} {3} {4} ${experiment_set_name_q} ${base_output_dir_q} ${experiment_output_dir_pattern_q} ${delete_output_dir_if_existing_q} ${extra_args}'" \
    ::: "${replanning_variants_ref[@]}" \
    ::: "${betas_ref[@]}" \
    ::: "${java_seed_indices_ref[@]}" \
    ::: "${rust_seeds_ref[@]}" || parallel_status=$?
