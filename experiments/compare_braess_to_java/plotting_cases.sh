...
plot_differences_per_seed_case() {
  local replanning_variant="$1"
  local seeds_to_avg_over="$2"
  local beta="$3"
  local seed_index="$4"
  ...
  if [ "${seeds_to_avg_over}" != "rust" ]; then
    echo "plot_differences_per_seed_case is only applicable when seeds_to_avg_over is 'rust', but got: $seeds_to_avg_over" >&2
    return 0
  fi
  ...
  echo "Plotting differences of average travel times and summed departures per seed for parameters: replanning_variant=${replanning_variant}, beta=$beta, read_from_random=$read_from_random, use_random_seed=$use_random_seed, seeds_to_avg_over=$seeds_to_avg_over"


  # Plot the average travel times and summed departures.
  if ! python python_plotting/braess/plot_sd_tt_single_seed_differences.py "$beta" "${replanning_variant}" "$read_from_random" "$use_random_seed" "varying_rust_seeds" "recreating_java_results" "$output_plots_dir"
  then
    echo "Plotting failed, continuing with next case." >&2
    record_failure plotting "$replanning_variant" "$beta" "$read_from_random" "$use_random_seed"
    return 1
...
plot_avg_over_rust_seeds_minus_original_java_case() {
  local replanning_variant="$1"
  local seeds_to_avg_over="$2"
  local beta="$3"
  ...
  echo "Plotting differences of average travel times and summed departures averaged over ${seeds_to_avg_over} seeds to original java data for parameters: replanning_variant=${replanning_variant}, beta=$beta, fixed_seed=$fixed_seed, seeds_to_avg_over=$seeds_to_avg_over"

  # Plot the average travel times and summed departures averaged over seeds.
  if ! python python_plotting/braess/plot_tt_and_sd_avg_over_rust_minus_java.py "$beta" "${replanning_variant}" "$fixed_seed" "recreating_java_results" "varying_rust_seeds" "$output_plots_dir"
  then
    echo "Plotting failed, continuing with next case." >&2
...
plot_once_per_replanning_variant_minus_java_case() {
  local replanning_variant="$1"

  local output_plots_dir="${SIM_OUTPUT_BASE_DIR}/${replanning_variant}/varying_rust_seeds/analysis/plots/deviations/"
  ...
  # Plot the average travel times and summed departures once per replanning variant.
  if ! python python_plotting/braess/plot_tt_and_sd_deviations_over_beta_rust_minus_java.py "${replanning_variant}" "recreating_java_results" "varying_rust_seeds" "$output_plots_dir"
  then
    echo "Plotting failed, continuing with next case." >&2
    record_failure plotting "$replanning_variant" "all_betas" "default fixed seed 1" "avg_over_rust_seeds"
    echo "Failed to plot deviations for replanning variant: $replanning_variant"
    return 1
  fi
